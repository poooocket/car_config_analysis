...
# 页面设置
st.set_page_config(page_title="车内视觉识别", layout="wide")
st.markdown("##### 车内视觉识别")
run_style()
...
df = config[["车型", "年款", "配置名称", "价格区间(万)", "厂商", "主动式DMS疲劳检测", "手势控制功能",  "车内摄像头数量(个)", "面部识别", "情绪识别"]]

col_names = ["主动式DMS疲劳检测", "手势控制功能", "面部识别", "情绪识别"]
for col in col_names:  
    df[col] = df[col].astype(str) + f"({col})"

df.loc[:, "has_cabin_camara"] = df["车内摄像头数量(个)"].notna().astype(int)
pannels = st.columns([1,2])
# 渗透率
with pannels[0]:
    with st.container(border=True): 
        st.markdown("###### 车内摄像头渗透率(按年款)")
        year = plot_penetration_by_year(df, "has_cabin_camara", percentage_height=percentage_height, key="year")
        
    with st.container(border=True): 
        st.markdown("###### 车内摄像头渗透率(按价格)")
        price_range = plot_penetration_by_price(df, "has_cabin_camara", percentage_height=percentage_height, key="price_range")

# 配置选型
with pannels[1]:     
    with st.container(border=True):
        cols = st.columns([4,1])
        cols[0].markdown("###### 厂商座舱视觉识别功能")
        sankey_cols = ["厂商","车内摄像头数量(个)", "主动式DMS疲劳检测", "手势控制功能", "面部识别", "情绪识别"]
